...
class SignInJwtUtilsIT extends BaseIT {
	...
	// SIGNUP USER:
		...
		// basic1
		MvcResult basic1SignUpResult = basic1SignUpResult();

			// retrieve nonce from basic1
			Cookie nonceCookie = basic1SignUpResult.getResponse().getCookie("COOKIE_NONCE");
			String encryptedNonce = objectMapper.readValue(basic1SignUpResult.getResponse().getContentAsString(), StateMessage.class)
		...
		// basic2
		MvcResult basic2SignUpResult = basic2SignUpResult(nonceCookie, encryptedNonce);


			// get token from basic2
			IStateMessage message = fromContent(basic2SignUpResult.getResponse().getContentAsString());
			String token = message.getMessage().substring("token".length()).trim();

			Cookie refreshTokenCookie = getRefreshTokenCookie(basic2SignUpResult);
	...
	// SIGNIN USER:
		...
		// basic1
		MvcResult basic1SignInResult = basic1SignUpResult();

		// retrieve nonce from basic1
		nonceCookie = basic1SignInResult.getResponse().getCookie("COOKIE_NONCE");
		encryptedNonce = objectMapper.readValue(basic1SignInResult.getResponse().getContentAsString(), StateMessage.class)
				.getMessage().substring("nonce".length()).trim();


		// basic2
		MvcResult basic2SignInResult = basic2SignUpResult(nonceCookie, encryptedNonce);


		// get token from basic2
		message = fromContent(basic2SignInResult.getResponse().getContentAsString());
		token = message.getMessage().substring("token".length()).trim();

		refreshTokenCookie = getRefreshTokenCookie(basic2SignInResult);

		assertThat(refreshTokenCookie).isNotNull();

		refreshToken1 = refreshTokenCookie.getValue();
	...
	private MvcResult basic2SignUpResult(Cookie nonceCookie, String encryptedNonce) throws Exception {
		// basic2
	...
	private MvcResult basic2SignInResult(Cookie nonceCookie, String encryptedNonce) throws Exception {
		// basic2
		final UriComponentsBuilder validateNonce = fromUriString("/signin/basic/2");

		LoginRequest loginRequest2 = new LoginRequest.Builder()
				.id(id)
				.password(password)
				.provider(providerType)
				.nonce(encryptedNonce)
	...
	private MvcResult basic1SignUpResult() throws Exception {
		// basic 1
	...
	private MvcResult basic1SignInResult() throws Exception {
		// basic 1
		final UriComponentsBuilder basic1 = fromUriString("/signin/basic/1");

		final LoginRequest loginRequest1 = new LoginRequest.Builder()
				.id(id)
				.provider(providerType)
				.build();
